...
//! ------------- The Game function wraps around the rest of the logic to be re-called on request -----------------
function game() {
  console.log(backgroundBlack);

//! ---------------------------------- Format the screen and display the title ------------------------------------
  start();
  async function start() {
    let guessWord = [
      reset + backgroundBlack,
      `${red}                                                                                   GGGGGGGGGGGGG`,
      `                                                                                 GGG::::::::::::G`,
      `                                                                                 GG:::::::::::::::G`,
      `                                                                                 G:::::GGGGGGGG::::G`,
  ...
  async function pickHighNum() {
    // console.log("\u001b[5B", "\u001b[5A")
    let awaitOrKeep = await ask(
      `                                                                                                          ${brightWhite}Would you like to ${brightYellow}(s)et ${brightWhite}the limit or ${brightYellow}(k)eep ${brightWhite}it at ${brightYellow}100${brightWhite}?\n                                                                                                                                                                                                                                                                                                                                                                                                                   `
    );
    // console.log("\n\n\n")
    if (awaitOrKeep == "k" || awaitOrKeep == "keep") {
      ...
      // secretNum
      highNum = await ask(
        `\u001b[2A\u001b[0J                                                                                                                          Please enter the ${brightYellow}number${brightWhite} \n                                                                                                                                     `
      );
      if (isNaN(highNum)) {
        console.log("\u001b[3A\u001b[0J");
        pickHighNum();
...
//! ----------- Ask which Game Mode the player wants, and either call higherOrLower() or playerGuesses() ----------
  ...
  async function higherOrLower() {
    secretNum = await ask(
      `                                                                                                                    ${brightWhite}Pick a number between ${brightYellow}${minNum} ${brightWhite}and ${brightYellow}${highNum}...\n                                                                                                                                    `
    );
    secretNum = parseInt(secretNum);
    if (isNaN(secretNum)) {
      console.log(
        `\u001b[3A\u001b[2K${brightWhite}                                                                                                                    Try again, and enter a \u001b[35;1mnumber.${cyan}`
      );
      higherOrLower();
    } else if (secretNum > highNum || secretNum < minNum) {
      console.log(
        `                                                                                                              \u001b[3A${brightWhite} Try to keep it between ${brightYellow}${minNum} ${brightWhite}and ${brightYellow}${highNum}, ${brightWhite}okay?${cyan}`
      );
      higherOrLower();
    } else {
      console.log(
        `                                                                                                                             \u001b[4A${brightWhite}\u001b[2K   You picked\n                                                                                                                                    ${brightYellow}${secretNum}                                                                                                                        \n\u001b[2K${brightWhite}                                                                                                                         Let the games begin!!!`
      );
      computerGuesses();
    }
  }
  ...
  async function playerGuesses() {
    ...
    if (isNaN(pickANumber)) {
      console.log(
        `\u001b[5A\u001b[0J                                                                                                               ${brightGreen}Congratulations${brightWhite}! You've found the ${brightMagenta}keyboard${brightWhite}!`
      );
      playerGuesses();
    } else if (pickANumber > highNum) {
      console.log(
        `\u001b[3A\u001b[0J                                                                                                                     Let's keep it ${cyan}under ${brightYellow}${highNum}${brightWhite}, okay?\n                                                                                                                                                                                                                                                                                                 `
      );
      ...
      console.log(
        `                                                                                                                           You've gone ${red}too low${brightWhite}! \n                                                                                                                    You have to keep your guess above ${brightYellow}${minNum}${brightWhite}.`
      );
      playerGuesses();
    } else if (pickANumber > rand) {
      console.log(
        `\u001b[5A\u001b[0J${brightWhite}                                                                                                                           Too ${brightCyan}High!${brightWhite} Try again.`
      );
      playerGuesses();
    } else if (pickANumber < rand) {
      console.log(rand);
      console.log(
        `\u001b[6A\u001b[0J${brightWhite}                                                                                                                         ${brightWhite}Too ${brightRed}Loooooow! ${brightWhite}Try again.`
      );
      playerGuesses();
    } else if (pickANumber == rand) {
      console.log(
        `\n\n\u001b[7A\u001b[0J                                                                                                                    ${brightWhite}You guessed it! The number is: ${brightYellow}${pickANumber}${brightWhite}!`
      ); // Add styling
      playAgain();
    } else {
  ...
  async function computerGuesses() {
    ...
    if (
      ...
      answerHighOrLow == "l" ||
      answerHighOrLow == "lower"
    ) {
      console.log(`\u001b[4A\u001b[2K${brightWhite}`);
      highNum = randNum - 1;

      if (minNum > secretNum) {
        let youSure = await ask(
          `                                                                                                                         You sure about that?                                                                                                                                                                                                                                                              ${brightYellow}(y)es${brightWhite}, or ${brightYellow}(n)o${brightWhite}?                                                                                                                                                                                                                                                                        `
        );

        if (youSure == "y" || youSure == "yes") {
          let reallySure = await ask(
            `                                                                                                                       Are you ${brightRed}SURE${brightWhite} you're ${brightYellow}sure${brightWhite}?                                                                                                                                                                                                                                                           ${brightYellow}(y)es${brightWhite}, or ${brightYellow}(n)o${brightWhite}?                                                                                                                    `
          );
        ...
        if (highNum == minNum) {
          console.log(
            `\n\n\u001b[5A\u001b[0J                                                                                                                          ${brightWhite}Your number is: ${brightYellow}${minNum}${brightWhite}!`
          ); // Add styling
          playAgain();
        } else if (highNum < 1) {
          console.log(
            `${brightWhite}                                                                                                                             ${brightWhite}Too ${brightWhite} ${brightYellow}low${brightWhite}, dude. \n[1m                                                                                                                  ${brightWhite}Did you ${brightWhite} ${brightYellow}forget ${brightWhite}your ${brightWhite} ${brightYellow}number???${brightWhite}\u001b[2K`
          );
          playAgain();
        } // Check the text codes after "number???"
      }
      ...
      if (minNum == highNum) {
        console.log(
          `                                                                                                        Well, it's either ${highNum}, or you hit the wrong button!`
        );
        playAgain();
      ...
      if (randNum == secretNum) {
        console.log(
          `\u001b[6A\u001b[2K${brightWhite}                                                                                                                     Woo hoo! ${brightWhite}Your number was ${brightYellow}${randNum}${brightWhite}!!!`
        );
        playAgain();
      } else {
        console.log(
          `\u001b[6A\u001b[2K${brightWhite}                                                                                                                     Nobody Likes a ${brightRed}Cheater${brightWhite}!!!`
        );
        process.exit();
      }
      ...
      console.log(
        `                                                                                                                       \u001b[4A\u001b[0J${brightWhite}${brightYellow}(h)igher${brightWhite}, or ${brightYellow}(l)ower${brightWhite}, please...`
      );
    }
    computerGuesses();
  }
